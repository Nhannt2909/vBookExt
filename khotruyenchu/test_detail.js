...
let detailCode = fs.readFileSync('src/detail.js', 'utf8').replace("load('config.js');", "");
detailCode = detailCode.replace(/let response = fetch\(url, \{[\s\S]*?\}\);/m, "let response = await fetch(url, {\n        headers: {\n            'user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36'\n        }\n    });");
detailCode = detailCode.replace("function execute(url) {", "async function execute(url) {");
